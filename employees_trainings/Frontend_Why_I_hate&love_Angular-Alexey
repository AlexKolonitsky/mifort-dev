...
8.	Узнав проблемный модуль нужно определить причину ошибки: проверяем правильность названия модуля, проверяем подключение файла модуля в index.html и корректность указанного к нему пути, проверяем сам Dependency Injection внутри модуля на ошибки в названиях, при использовании array notation проверяем количество подключаемых зависимостей вне массива зависимостей и внутри него
